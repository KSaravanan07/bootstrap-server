...
void serve(int FileServertype)//Registartion of the Fileserver
{
 	/*
        ...
        4->Mp4/mp3
        */
    if(FileServertype == 1)//the File server is of pdf type
    {   
        int port;
        ...
        status = 1; // registered successfully
        sendto(sockfd,&status,sizeof(int),0,(struct sockaddr*)&sockaddr1, sizeof(sockaddr1));
    }
...
int main(){
  ...
  addr_size = sizeof(sockaddr1);
   ...
   while(1) //we have taken an infinite for loop so the server never stops
   {
   /*
